...
class MoveAttach(var fragment: Frag_SearchAddFile, var objAttachItem:JsonObject):AsyncTask<JsonObject, Void, JsonObject>(){
    ...
    override fun onPreExecute() {
        ...
        builder = AlertDialog.Builder(activity as Context).apply {
            ...
            view.findViewById<TextView>(R.id.view_textProgressTitle).text = activity.getString(R.string.in_progress_move_attach)
        ...
        if(!isCancelled)
        {
            ...
            if (m_C.isNetworkConnected(activity as Context)) {
                ...
                if ("SOCKET".equals(CONNECTION_PROTOCOL, true)) {
                    if(SocketManager().moveAttach(objAttachItem ,objUserInfo))
                    {
                        nCurrentStatus = Frag_SearchAddFile.MOVE_ADDFILE_SUCCESS
                    }
                }
            }
            else
    ...
    override fun onPostExecute(result: JsonObject) {
        ...
        when(nCurrentStatus) {
            Frag_SearchAddFile.MOVE_ADDFILE_SUCCESS -> {
                ...
                AlertDialog.Builder(activity as Context).run {
                    setMessage(activity.getString(R.string.success_move_attach))
            ...
            Frag_SearchAddFile.MOVE_ADDFILE_FAILED -> {
                AlertDialog.Builder(activity as Context).run {
                    setMessage(activity.getString(R.string.failed_move_attach))
                    setPositiveButton(activity.getString(R.string.btn_confirm)) { dialog, which ->
                        //    activity.onBackPressed()
                    }
                }.show()
