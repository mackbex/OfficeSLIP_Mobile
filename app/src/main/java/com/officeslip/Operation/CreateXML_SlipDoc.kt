...
import com.officeslip.AGENT_FOLDER
import com.officeslip.CONNECTION_CHARSET
import com.officeslip.TEMP_PATH
...
class CreateXML_SlipDoc {
    ...
    //Create DocData
    fun getElement_DocData(activity:Activity?):Element? {
        var elDocData:Element? = null
        activity?.run {
            assets.open("common/SLIPDOC.xml")?.run {
    ...
    //Create DocList node
    fun getElement_DocListItem(elDocList:Element?, arObjImages: JsonArray, strSDocNo: String, isSDocOne:Boolean):Element?
    {
        ...
        if(isSDocOne)
        {
            ...
            for (obj in arObjImages)
            {
                ...
                var strDocIRN = obj.get("DocIRN").asString
            ...
            elDoc.setAttribute("slip_no",m_C.getSlipNo(4,nPage))
            ...
            for (obj in arObjImages) {
                ...
                elDoc.setAttribute("cabinet", m_C.getDate("yyyyMMdd"))
                elDoc.setAttribute("Folder", AGENT_FOLDER)
                elDoc.setAttribute("doc_irn", obj.get("DocIRN").asString)
                elDoc.setAttribute("sdoc_no", strSDocNo)
                elDoc.setAttribute("slip_no", m_C.getSlipNo(4, nPage))
                elDoc.setAttribute("Comment", "SLIP")
                elDoc.setAttribute("File", obj.get("DocIRN").asString + ".J2K")
                elDoc.setAttribute("file_cnt", "1")
                elDoc.setAttribute("doc_status", "1")
                elDoc.setAttribute("bCompress", "1")
                elDoc.setAttribute("slip_flag", "0")
                elDoc.setAttribute("file_size", obj.get("ImageSize").asString)
                elDoc.setAttribute("update_time", m_C.getDate("yyMMddhhmmsss"))
    ...
    //Create DocList node
    fun getElement_DocListItemForCopy(elDocList:Element?, arObjImages: JsonArray, strSDocNo: String, isSDocOne:Boolean):Element?
    {
        if(arObjImages == null) return null
        ...
        if(isSDocOne)
        {
            ...
            for (obj in arObjImages) {
                ...
                elDoc.setAttribute("cabinet", obj.get("CABINET").asString)
                elDoc.setAttribute("Folder", obj.get("FOLDER").asString)
                elDoc.setAttribute("doc_irn", obj.get("DOC_IRN").asString)
                elDoc.setAttribute("sdoc_no", strSDocNo)
                elDoc.setAttribute("slip_no", obj.get("SLIP_NO").asString)
                elDoc.setAttribute("Comment", "SLIP")
                elDoc.setAttribute("File", obj.get("DOC_IRN").asString + ".J2K")
                elDoc.setAttribute("file_cnt", obj.get("FILE_CNT").asString)
                elDoc.setAttribute("doc_status", "1")
                elDoc.setAttribute("bCompress", "1")
                elDoc.setAttribute("slip_flag", "0")
                elDoc.setAttribute("file_size", obj.get("FILE_SIZE").asString)
                elDoc.setAttribute("update_time", obj.get("UPDATE_TIME").asString)
    ...
    //Fill DocData items
    fun setDocData(elDocData:Element?, objSlipDocInfo:JsonObject, arObjImages:JsonArray, strSDocNo:String, strDocIRN:String, isSDocOne:Boolean):Element? {
        ...
        for(el in listElItem)
        {
            ...
            when(strElName)
            {
                ...
                "SDOC_TYPE" -> {
                    el.text = objSlipDocInfo.get("SDOC_TYPE").asJsonObject.get("code").asString
                }
                "SDOC_ONE" -> {
                ...
                "CO_NO" -> {
                    el.text = g_UserInfo.strCoID
                }
                "PART_NO" -> {
                    el.text = g_UserInfo.strPartID
                }
                "HG_DATE" -> {
                    el.text = m_C.getDate("yyyyMMdd")
                }
                "SLIP_CNT" -> {
                    el.text = if(isSDocOne) "1" else arObjImages.size().toString()
                }
                "FILE_CNT" -> {
                    el.text = arObjImages.size().toString()
                }
                "FILE_SIZE" -> {
                    el.text = m_C.getSlipTotalSize(arObjImages).toString()
                }
                "REG_USER" -> {
                    el.text = g_UserInfo.strUserID
                }
                "REG_USERNM" -> {
                    el.text = g_UserInfo.strUserName
                }
                "REG_TIME" -> {
    ...
    fun getElement_PageList(arObjImages: JsonArray, isSDocOne: Boolean):ArrayList<Element>? {
        ...
        for(obj in arObjImages)
        {
            ...
            elPage.setAttribute("Rect","0,0,"+m_C.pixel2pt(obj.get("Width").asString.toInt())+","+m_C.pixel2pt(obj.get("Height").asString.toInt()))
    ...
    fun getElement_PageListForCopy(arObjImages: JsonArray, isSDocOne: Boolean):ArrayList<Element>? {
        ...
        for(obj in arObjImages)
        {
            ...
            try {
                ...
                Logger.WriteException(this@CreateXML_SlipDoc::javaClass.name, "getElement_PageListForCopy",e,5)
            }
    ...
    fun write(elDocument:Element, strDocIRN:String):String?
    {
        ...
        var strSavePath = TEMP_PATH + File.separator
        ...
        FileOutputStream(file).use {
            ...
            format.encoding = CONNECTION_CHARSET
            format.indent = "  "
